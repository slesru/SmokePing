package Smokeping::probes::TelnetAPOSPing;

=head1 301 Moved Permanently

This is a Smokeping probe module. Please use the command 

C<smokeping -man Smokeping::probes::TelnetAPOSPing>

to view the documentation or the command

C<smokeping -makepod Smokeping::probes::TelnetAPOSPing>
...
sub pod_hash {
	return {
		name => <<DOC,
Smokeping::probes::TelnetAPOSPing - Addpac APOS Probe for SmokePing
DOC
		description => <<DOC,
Integrates Addpac APOS as a probe into smokeping.  Uses the telnet protocol 
...
sub ProbeDesc($){
    ...
    return "InfoAve Addpac APOS - ICMP Echo Pings ($bytes Bytes)";
}
...
#     $telnet->print("ping " -c 20 ".$dest." ".$bytes);
     @output = $telnet->cmd("ping ".$dest." ".$bytes." 20");
...
#	$outputline =~ /^Reply to request \d+ \((\d+) ms\)/ && push(@times,$1);
	$outputline =~ /^\d+ bytes from $dest: icmp_seq=\d+ ttl=\d+ time = (\d+) ms$/ && push(@times,$1);
